...
@pytest.mark.asyncio
async def test_edit_entry_posts_patches_every_persisted_post(
  tmp_path,
  monkeypatch,
  dual_channels,
):
  """Live 2026-09-04: a trailed stop only ever went out as a reply - the
  pinned entry card kept showing its original SL/risk forever. Every
  persisted post must get its own in-place edit with the live sl value.
  """
  signal = await _signal(tmp_path, monkeypatch, "vip")
  send = AsyncMock(return_value=SimpleNamespace(message_id=101))
  monkeypatch.setattr(broadcast, "_send_message", send)
  ...
  edit = AsyncMock()
  monkeypatch.setattr(broadcast, "edit_message_text", edit)
  trailed = {**signal, "sl": 1996.0}

  await broadcast.edit_entry_posts(trailed)

  edit.assert_awaited_once_with(
    VIP_ID, 101, broadcast.render_entry(trailed, "vip"),
  )
  assert "1,996" in edit.await_args.args[2]


@pytest.mark.asyncio
async def test_post_result_sl_action_patches_pinned_card(monkeypatch):
  signal = {
    "id": 1,
    "daily_seq": 7,
    "status": "open",
  ...
  monkeypatch.setattr(trade_ops, "edit_entry_posts", edit)
  monkeypatch.setattr(
    trade_ops, "get_manual_signal", AsyncMock(return_value=signal),
  )

  await trade_ops.post_result({
    "action": "sl",
    "ok": True,
    "row": signal,
    "price": 1996.0,
  ...
  edit.assert_awaited_once_with(signal)


@pytest.mark.asyncio
async def test_post_result_non_sl_action_does_not_touch_pinned_card(
  monkeypatch,
):
  signal = {
    "id": 1,
  ...
  await trade_ops.post_result({
    ...
    "ok": True,
    "row": {
      "id": 1, "daily_seq": 7, "closed": False, "frac": 0.5, "remaining": 0.5,
    },
    "pips": 40,
  }, "XAU")
