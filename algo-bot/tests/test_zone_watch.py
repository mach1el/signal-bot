...
async def test_repeated_touches_downgrade_but_do_not_alone_exhaust_the_zone(
  ...
  """Touch count alone must never terminally invalidate/expire a zone."""
  zone_id = _zone_id()
  await zw.discover_zone_watch(
    client, zone_id=zone_id, symbol="XAU", direction="SELL",
    low=4113.0, high=4116.0, source_timeframe="M5",
  ...
  assert after_3.state not in zw.TERMINAL_ZONE_WATCH_STATES
  assert zw.is_actively_watchable(after_3)

  # Far beyond the old stale-touch safety net — still not terminal.
  record = after_3
  for _ in range(10):
    record = await zw.record_zone_touch(client, zone_id)
  assert record.touch_count == 13
  assert record.state not in zw.TERMINAL_ZONE_WATCH_STATES
  assert zw.is_actively_watchable(record)


@pytest.mark.asyncio
async def test_decisive_break_invalidates_the_zone_immediately(client):
  """A close beyond the zone's far/invalidating edge structurally
  invalidates — unlike a bounce back out the near edge.
  """
  zone_id = _zone_id()
  await zw.discover_zone_watch(
    client, zone_id=zone_id, symbol="XAU", direction="SELL",
  ...
  assert entered.state != zw.INVALIDATED
...
async def test_a_valid_bounce_exit_does_not_exhaust_the_zone(client):
  ...
  assert bounced.state not in zw.TERMINAL_ZONE_WATCH_STATES


@pytest.mark.asyncio
async def test_htf_evidence_prevents_invalidation_from_a_decisive_break(client):
  zone_id = _zone_id()
  await zw.discover_zone_watch(
    client, zone_id=zone_id, symbol="XAU", direction="SELL",
