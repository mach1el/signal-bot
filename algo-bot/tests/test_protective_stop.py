...
def test_fvg_collapses_to_single_entry_when_group_stop_cannot_fit():
  # 5-price FVG, ATR 8 → 0.5 ATR scale step is 4.0 price (40 pips) between
  # legs. Owner envelope slack is 20 pips, so a shared group SL cannot cover
  # both clips. scale_undersized_policy=single_entry must publish proximal.
  result = evaluate_execution_policy(
    _policy_subject(
      strategy="FVG",
      direction="SELL",
      entry_low=4420.0,
      entry_high=4425.0,
      current_price=4421.0,
      confluence=3,
      atr=8.0,
      structure_swing=4425.5,
      targets_pips=(60,),
      target_model="fill_relative",
    ),
    spot_price=4421.0,
    ...
    executable_quote=4421.0,
  )
  assert result.allowed, (result.reason_code, result.message, result.measured)
  assert result.measured["collapsed_to_single_entry"] is True
  assert len(result.measured["planned_leg_entry_prices"]) == 1
  assert result.measured["planned_stop_error"] is None
