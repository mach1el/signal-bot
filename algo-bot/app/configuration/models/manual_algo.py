...
class ManualAlgoSizingConfig(FrozenConfigModel):
    ...
    fx_volume_multiplier: float = config_field(1.5, canonical_env='MANUAL_ALGO_FX_VOLUME_MULTIPLIER', owner=ConfigOwner.SHARED, reload=ReloadPolicy.NEW_SETUP_ONLY, runtime_reload=ReloadPolicy.RESTART, unit=ConfigUnit.MULTIPLIER, risk=RiskClassification.EXECUTION_SAFETY, shared_with_ctrader=False, description='Scales equity-table lots for fixed_rr (FX) instruments: manual /algo candidates and autonomous reaction plans via instrument manual.risk_multiplier.', default_contexts=(ContextDefault(DefaultContext.PYTHON_SCHEMA, 1.5),), validation_summary='Must be positive finite')
