...
class ExecutionEntryConfig(FrozenConfigModel):
    ...
    max_spread_pips: int = config_field(5, canonical_env='AUTO_TRADE_MAX_SPREAD_PIPS', owner=ConfigOwner.CTRADER, reload=ReloadPolicy.NEW_SETUP_ONLY, runtime_reload=ReloadPolicy.RESTART, unit=ConfigUnit.PIPS, risk=RiskClassification.EXECUTION_SAFETY, description='cTrader configuration option AUTO_TRADE_MAX_SPREAD_PIPS controlling  (pips).', default_contexts=(ContextDefault(DefaultContext.CTRADER_FROM_ENVIRONMENT, 5),), validation_summary='EnvironmentResolver.Int + AutoTradeOptions.Validate')
