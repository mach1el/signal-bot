"""Quarantined legacy pydantic-settings configuration surface.

Phase 2H isolates the historical flat ``Settings`` model here so the
composition root in ``app.core.config`` no longer defines a
``pydantic_settings.BaseSettings`` subclass. The class body, validators,
aliases, computed properties, and profile semantics are preserved verbatim so
the legacy authority remains behaviourally identical and selectable for
rollback. Production code must not import this module directly; it consumes
``runtime_config`` through the composition root instead.

The class name stays ``Settings`` so the legacy authority reports the same
runtime type and startup diagnostics as before; ``LegacySettings`` is provided
as an explicit quarantine alias.
"""
...
from app.configuration.source_policy import PYTHON_SOURCE_POLICY


class Settings(BaseSettings):
  # Same dotenv file/encoding contract as the canonical source policy so both
  # authorities observe an identical file-source surface.
  model_config = SettingsConfigDict(
    env_file=PYTHON_SOURCE_POLICY.env_file,
    env_file_encoding=PYTHON_SOURCE_POLICY.env_file_encoding,
    extra="ignore",
  )
...
# Explicit quarantine alias. The active class name stays ``Settings`` to keep
# the legacy runtime type and startup diagnostics identical.
