...
)
{
  ...
  )
  {
    ...
    await ConvertPendingLimitsOnZoneTapAsync(
      client, symbol, quote, cancellationToken
    );
  ...
  )
  {
    ...
    var widthTolerance = (zoneHigh - zoneLow) * RecoveryCatchUpToleranceFraction;
    var chaseTolerance = Math.Max(0m, options.MaxEntryDistancePips) * options.PipSize;
    var tolerance = Math.Max(widthTolerance, chaseTolerance);
    // Use the live [bid, ask] span, not a single price_side. A BUY limit
    // ladder has no price_side; the bid-only check used to miss a tap that
    // the ask still overlapped.
    return quote.Ask >= zoneLow - tolerance && quote.Bid <= zoneHigh + tolerance;
  }

  private static bool QuoteOverlapsZone(SpotPrice quote, decimal zoneLow, decimal zoneHigh) =>
    quote.Ask >= zoneLow && quote.Bid <= zoneHigh;

  private static bool TryResolveEntryZone(
    TradePlan plan,
    out decimal zoneLow,
    out decimal zoneHigh
  )
  {
    if (plan.Entry.ZoneLow is decimal low && plan.Entry.ZoneHigh is decimal high && high > low)
    {
      zoneLow = low;
      zoneHigh = high;
      return true;
  ...
  private async Task ConvertPendingLimitsOnZoneTapAsync(
    ICTraderTradeClient client,
    SymbolInfo symbol,
    SpotPrice quote,
    CancellationToken cancellationToken
  )
  {
    ...
    foreach (var initial in _statesById.Values.ToArray())
    {
      ...
      if (!TryResolveEntryZone(plan, out var zoneLow, out var zoneHigh))
      {
        continue;
      }
      var l1 = (initial.Legs ?? []).FirstOrDefault(leg =>
        leg.LegId == "L1"
      ...
      var spreadTicks = StopTrailPlanner.RequireTickSize(symbol) > 0
        ...
        : 0m;
      if (
        plan.Entry.MaxSpreadTicks is int maxSpread
        && spreadTicks > maxSpread
      )
      {
        continue;
      }
      var tappedNow = QuoteOverlapsZone(quote, zoneLow, zoneHigh);
      if (
        !tappedNow
        && !await TryRecoveryCatchUpAsync(plan, quote, cancellationToken)
      )
      {
        continue;
      }
      log(
        $"v8 zone tap: id={plan.PlanId} converting L1 limit to market "
        + (tappedNow ? "live quote overlaps entry zone" : "M1 tapped zone")
      );
      await ConvertLimitLegToMarketAsync(
        client, symbol, plan, initial, l1, quote, cancellationToken
      );
    }
  }

  private async Task ConvertLimitLegToMarketAsync(
    ICTraderTradeClient client,
    SymbolInfo symbol,
    TradePlan plan,
    TradePlanRuntimeState state,
  ...
  )
  {
    try
    {
      await client.CancelPendingOrderAsync(leg.BrokerOrderId!.Value, cancellationToken);
    }
    catch (Exception exception)
    {
      log(
        $"v8 zone tap cancel failed id={plan.PlanId} leg={leg.LegId} "
        + $"exception={exception.GetType().Name}"
      );
      return;
    }
    ...
    try
    {
      ...
      var filled = new TradePlanLegRuntimeState(
        LegId: leg.LegId,
        IntendedPrice: entryPrice,
        DeclaredRatio: leg.DeclaredRatio,
        IntendedVolume: leg.IntendedVolume,
        IntendedLots: leg.IntendedLots,
      ...
      UpsertLeg(legs, filled);
      var pendingIds = (state.PendingOrderIds ?? [])
        .Where(id => id != leg.BrokerOrderId)
        .ToArray();
      var next = AggregateState(
        state with
        {
          Legs = legs.ToArray(),
          PendingOrderIds = pendingIds.Length == 0 ? null : pendingIds,
        }
      );
      next = await AmendAndVerifyLegStopAsync(
        client, symbol, next, leg.LegId, plan.Stop.Price, cancellationToken
      );
      await PersistStateAsync(next, cancellationToken);
      await PersistPlanExecutionStateAsync(
        plan.PlanId,
        next.Stage is TradePlanRuntimeStage.FullyOpen or TradePlanRuntimeStage.Open
          ? "filled"
          : "submitted",
        null,
        cancellationToken
    ...
    {
      log(
        $"v8 zone tap market failed id={plan.PlanId} "
        + $"exception={exception.GetType().Name} message={exception.Message}"
      );
    }
  }
